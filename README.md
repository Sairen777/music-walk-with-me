...
Two capsules ship today, **USA** and **Russia** (2004-2007 each): the landing is a
...
- `scripts/hydrate.mjs` — queries the public iTunes Search API per track, keeps the
  ...
  (just `iso`, `countryName`, `years` — no track data).
- The app fetches `index.json` once for the map, then fetches a country's shard **only
  when you click it** (cached after). No track data is bundled into the JS, so it scales
  to many countries without bloating the initial load. If one country ever gets huge,
  split its shard by decade the same way.
